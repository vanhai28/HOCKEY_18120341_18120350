...
{
	if (is_move && rect_.x < x_ball_predict && x_ball_predict < (rect_.x + WIDTH_PLAYER))
	{
		is_move = false;
	}
	else if (is_move)
	{
		if (rect_.x + WIDTH_PLAYER <= x_ball_predict && rect_.x + WIDTH_PLAYER + x_step < RIGHT_BOUNDARY)
		{
			rect_.x += x_step;
		}
		else if (rect_.x >= x_ball_predict && rect_.x - x_step > LEFT_BOUNDARY)
		{
			rect_.x -= x_step;
		}
	}
